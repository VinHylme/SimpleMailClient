...
public interface InterfaceClientModel {
	...
	boolean theLastMessage(InterfaceMessage theMessage);

	InterfaceMessage getTheLastMessage();

	boolean flagMessages(String colour, int messageId);

	boolean deleteFolder(String name);

	void undoFun(AbstractCommand command);



	String excUndo(AbstractCommand command);

	boolean excRedo(AbstractCommand command);

	String theRules(String folderName, String theType, String searchFor);

	boolean checkRules();

	String aliasComa(String oldComa, String newComa);

	String removeAlias(String oldComa, String newComa);
